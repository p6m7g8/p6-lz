...
export async function getAccounts(logger: winston.Logger, client: S3Client, accountsFileBucket: string, accountsFileKey: string): Promise<P6LzAccounts> {
...
async function parseAccounts(logger: winston.Logger, s3Object: GetObjectCommandOutput): Promise<P6LzAccounts> {
  let accountData: P6LzAccounts
  try {
    const bodyString = await streamToString(s3Object.Body as Readable)
